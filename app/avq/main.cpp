...
int main(int argc, char** argv)
{
  ...
  if(args.additional_compression)
  {
      if(!args.mode)
      {
          io_handler = new IO_Handler(args.in_filename, args.out_filename, AC_ENCODING);
      }else
      {
          io_handler = new IO_Handler(args.in_filename, args.out_filename, AC_DECODING);
      }
  } else
  {
      io_handler = new IO_Handler(args.in_filename, args.out_filename, NO_ADDITIONAL_COMPRESSION);
  }
